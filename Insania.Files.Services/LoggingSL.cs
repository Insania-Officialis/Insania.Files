...
using Insania.Files.Contracts.Services;
using Insania.Files.Database.Contexts;
using Insania.Files.Entities;

namespace Insania.Files.Services;
...
public class LoggingSL: ILoggingSL, IHostedService, IDisposable
{
    ...
    public LoggingSL(IServiceScopeFactory scopeFactory, ILogger<LoggingSL> logger)
    {
        ...
        //Создание канала
        _channel = Channel.CreateBounded<LogApiFiles>(options);
    }
    #endregion
    ...
    /// </summary>
    private readonly Channel<LogApiFiles> _channel;
    ...
    /// Пачка записей
    /// </summary>
    private readonly List<LogApiFiles> _batch = [];
    ...
    public ValueTask QueueLogAsync(LogApiFiles log) => _channel.Writer.WriteAsync(log, _cancellationTokenSource.Token);
    ...
    private async Task SaveBatchAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogsApiFilesContext context = scope.ServiceProvider.GetRequiredService<LogsApiFilesContext>();
